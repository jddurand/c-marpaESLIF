...
If the destination charset compares to the source charset, then tconv() will I<trust> the parameters and do direct byte copy, simulating iconv() behaviour. Therefore it is desirable to specify the input charset when you can, or at least give a large enough first input buffer so that encoding guess is reliable.
