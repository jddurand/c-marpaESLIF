...
/****************************************************************************/
{
  ...
  MARPAWRAPPER_TRACEF(genericLoggerp, funcs, "marpa_r_terminal_is_expected(%p, %d)", marpaWrapperRecognizerp->marpaRecognizerp, symboli);
  ...
  isExpectedb = (isExpectedi > 0) ? 1 : 0;

  if (isExpectedbp != NULL) {
    *isExpectedbp = isExpectedb;
  }
