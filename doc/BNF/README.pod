...
Lexemes are meta-symbols that does appear as the LHS symbol anywhere within the current grammar. Therefore they behave like terminals, except that their definition is not in the grammar. By default such meta-symbol is looked up in the grammar at the next level. For example:
