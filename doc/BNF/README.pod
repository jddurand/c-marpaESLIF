...
They are always enclosed with left and right brackets C<[]>, and the content must be valid as per the <PCRE2 Perl Compatible Regular Expression|http://www.pcre.org/> library. Modifiers can start after a C<:> character. A character class class is nothing else but a lexically restricted regular expression.

=item * Regular expression

They are always enclosed with slashes C<//>, and the content must be valid as per the <PCRE2 Perl Compatible Regular Expression|http://www.pcre.org/> library. Modifiers can start after the slash on the right. Regular expression patterns are by default anchored.
...
The PCRE2 syntax is supported in its entirety, this include any PCRE2 add-on. Character classes and regular expression share the same set of modifiers, executed in order of appearance, that are:
  ...
  ----------------------------------------------------------------
  s                                    PCRE2_DOTALL
  A dot meta-character in the pattern matches all characters, including newlines
  ----------------------------------------------------------------
  x                                    PCRE2_EXTENDED
  Enable comments. This has some limitation due marpaESLIF semantics
  ----------------------------------------------------------------
  D                                    PCRE2_DOLLAR_ENDONLY
  A dollar meta-character matches only at the end of the subject string
  ----------------------------------------------------------------
  J                                    PCRE2_DUPNAMES
  ...
  ----------------------------------------------------------------
  A                 PCRE2_ANCHORED
  Remove the systematic anchoring
  ----------------------------------------------------------------
