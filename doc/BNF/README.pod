...
Hook that is disabling C<:discard> rule for the current recognizer. Equivalent to a call to C<marpaESLIF_recognizer_hook_discardb(1)>. Not propagated.

=item C<:discard[off]>

Hook that is enabling C<:discard> rule for the current recognizer. Equivalent to a call to C<marpaESLIF_recognizer_hook_discardb(0)>. Not propagated.
