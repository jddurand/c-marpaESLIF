...
The C<terminals-are-exclusive> adverb indicates ESLIF that once a grammar terminal match there is no need to try any other grammar terminal. This can speed up significantly the automatic scanning, but requires the end-user to be I<sure> that all grammar terminals (i.e. direct terminals or lexemes) cannot match the same thing. Note that it affects also the :discard meta symbol.
...
marpaESLIF BNF can be expressed in itself:
  ...
  <adverb item>                  ::= <action>
                                   ...
                                   | <terminals are exclusive specification>
                                   | <naming>
                                   | <null adverb>
                                   | <symbol action>
                                   | <event specification>
  ...
  <terminals are exclusive specification> ::= 'terminals-are-exclusive' /=>|\x{21D2}/u <false>
                                        | 'terminals-are-exclusive' /=>|\x{21D2}/u <true>
