...
Itapplies I<only> when the symbol is discovered via the automatic scan: The C<if-action> must refer to a callback in user-land's recognizer interface or in embedded lua interpreter, and must return a boolean:
...
It a transversal grammar setting; and can be set only via C<::default> meta rule:
...
and to symbols in the form:

  :symbol ::= /X/ name => XRegexp
  :symbol   ::= X   name => XLexeme
...
marpaESLIF BNF can be expressed in itself:
  ...
  <symbol rule>                  ::= ':symbol' <op declare> <terminal> <adverb list>
