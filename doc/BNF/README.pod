...
The I<exception statement> is also an exception from syntax point of view, its semantic is:

  LHS ::= RHS1 - RHS2

And empty rule have no RHS:

  EMPTYRULE ::=

Ambiguities in the grammar itself may be solve by adding the C<;> character at the end of a rule, or by enclosing zero or more statements within C<{> and C<}> characters:
...
As you can see statements has been grouped at every occurence of C<||> operator. Therefore the I<loosen> operator C<||> is a I<convenience> operator, it is always possible to write an equivalent grammar without it, though this can become quite tedious.
