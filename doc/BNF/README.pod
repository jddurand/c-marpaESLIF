...
=item Setting default actions

By default, symbol action is C<::shift> and rule action is C<::concat>, i.e. the parse tree value of a grammar is a concatenation of every matched input, without the eventual discard. Stack manipulation may require the trigger of a I<free> function, and this has no default. Only expected terminals or lexemes are looked up, this is the Longest Acceptable Token Match (LATM) setting. These can be set once, for example like this:
...
For example:
  ...
  event b=off = completed symbolname2
  event c=on  = completed symbolname3
