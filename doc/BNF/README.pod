...
A concatenation rule that tries to keep the nature of the RHS:

=over

=item In symbol context

If the symbol was fed with a user alternative, the later is returned, else return value is always of type ARRAY.

=item In rule context

If there is a single RHS, the later is returned, else returns a value of type ARRAY from the binary concatenation all the RHS's representation, or UNDEF if global representation is empty or if it is nullable.
