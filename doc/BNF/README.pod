...
Please note that the C<:discard[on]>, C<:discard[off]> and C<:discard[switch]> events will I<always> happen if specified in the grammar with an true initial state, and at I<any level> (i.e. including lexeme sub grammars). No callback to the end user will happen. This is because these are internal events, categorized as parsing hooks.
