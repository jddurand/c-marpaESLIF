...
Puts in C<versionsp>, if not C<NULL>, a pointer to a C<NUL> terminated I<ASCII> string containing ESLIF version, in the form C<MAJOR.MINOR.PATCH>.
...
=head3 C<marpaESLIF_grammarp>

  marpaESLIFGrammar_t *marpaESLIF_grammarp(marpaESLIF_t *marpaESLIFp);

Returns the ESLIF grammar itself, or C<NULL> if failure. The returned pointer must I<never> be freed.

=head3 C<marpaESLIF_numberb>
...
Tries to parse the I<ASCII> string pointed by C<s> of size C<sizel> bytes (if C<sizel> is 0, ESLIF will compute automatically the length) as a JSON relaxed number, i.e. with possible leading C<+> and zeroes. Returns in C<marpaESLIFValueResultp>, when not C<NULL>, the result, and in C<confidencebp>, when not C<NULL>, a true number if ESLIF is sure. The value type in C<marpaESLIFValueResultp> is guaranteed to be one of these:
...
=item MARPAESLIF_VALUE_TYPE_LONG_LONG

=item MARPAESLIF_VALUE_TYPE_LONG_DOUBLE

=item MARPAESLIF_VALUE_TYPE_UNDEF

=back 

C<MARPAESLIF_VALUE_TYPE_UNDEF> indicates a failure, and always have C<*confidencebp> to C<0>.

Returns a true value on success, a false value otherwise.

=head3 C<marpaESLIFValueResult_isinfb>

  short marpaESLIFValueResult_isinfb(marpaESLIF_t *marpaESLIFp, marpaESLIFValueResult_t *marpaESLIFValueResultp);

Returns a true value if C<marpaESLIFp> and C<marpaESLIFValueResultp> arguments are correct and if C<marpaESLIFValueResultp> hosts an infinity number.

=head3 C<marpaESLIFValueResult_isnanb>

  short marpaESLIFValueResult_isnanb(marpaESLIF_t *marpaESLIFp, marpaESLIFValueResult_t *marpaESLIFValueResultp);

Returns a true value if C<marpaESLIFp> and C<marpaESLIFValueResultp> arguments are correct and if C<marpaESLIFValueResultp> hosts a NaN.

=head3 C<marpaESLIFValueResult_is_positive_nanb>
...
Returns a true value if C<marpaESLIFp> and C<marpaESLIFValueResultp> arguments are correct and if C<marpaESLIFValueResultp> hosts a positive NaN number. ESLIF returns a true value in C<confidenceb> when it is sure, a false otherwise. The false value happens when getting the C<NaN> is not sure about the sign.

=head3 C<marpaESLIFValueResult_is_negative_nanb>
...
Returns a true value if C<marpaESLIFp> and C<marpaESLIFValueResultp> arguments are correct and if C<marpaESLIFValueResultp> hosts a negative NaN number. ESLIF returns a true value in C<confidenceb> when it is sure, a false otherwise. The false value happens when getting the C<NaN> is not sure about the sign.
...
=item C<MARPAESLIF_ACTION_TYPE_LUA_FUNCTION>
...
Lua 5.3.4 semantics are applied I<immediately> after the opening parenthesis, up to the end of a function body parsing as per the Lua 5.3.4 grammar. ESLIF semantics takes over after the C<end> lua keyword that corresponds to the end of the lua function.
...
=item C<marpaESLIFRecognizer_newp>
...
A short form of parsing and valuation, in which I<no event> but I<if> or I<regexp> callback is possible: the entire scanning is left to the marpaESLIF engine. The grammar at level C<0> is used.
...
=item MARPAESLIF_VALUE_TYPE_ROW

Value type is C<marpaESLIFValueResultRow_t>, naively corresponding the notion of array in any traditional language, and typedef'ed to:
...
where C<p> is an array of C<marpaESLIFValueResult_t> values, of size C<sizel>, that can be zero.

=item MARPAESLIF_VALUE_TYPE_TABLE

Value type is C<marpaESLIFValueResultTable_t>, naively corresponding the notion of hash in any language that supports it, i.e. a key and a value, typedef'ed to:
...
where C<p> is an array of pairs of C<marpaESLIFValueResult_t> key and value, of size C<sizel>, that can be zero.

In contrary to most languages, the key can be of any type, it is not restricted to a string.

=item MARPAESLIF_VALUE_TYPE_LONG_DOUBLE

Value type is C<marpaESLIFValueResultLongDouble_t>, typedef'ed to: C<long double>.

=item MARPAESLIF_VALUE_TYPE_LONG_LONG

Value type is C<marpaESLIFValueResultLongLong_t>, typedef'ed to: C<long long>, when your compiler has support for this native C type.
...
=item C<disposeCallbackpp>

A uesr-defined function pointer that, if not C<NULL>, is guaranteed to be called as soon as ESLIF do not need anymore what the reader returned.
...
If a lexeme or a terminal is a candidate for automatic parsing (I<scan> or I<parse> modes), this callback must not be C<NULL> and return a function pointer. The later will be be called with a C<marpaESLIFValueResultp> argument that is guaranteed to be of type C<ARRAY>, and must return in C<marpaESLIFValueResultBoolp>, guaranteed to not be C<NULL>, a boolean value saying of the lexeme or terminal can be taken in account in the parse tree.

The function will return a true value in case of success, a false value otherwise.

=item C<eventActionResolverp>

If the grammar has set a generic event callback via C<:default event-action>, this callback must not be C<NULL> and return a function pointer. The later will be be called with a C<actions> pointer containing an C<NUL> terminated ASCII string representing the name of the event action.

=item C<regexActionResolverp>

If the grammar has set a generic regular expression callback via C<:default regex-action>, this callback must not be C<NULL> and return a function pointer. The later will be be called with a C<marpaESLIFCalloutBlockp> pointer containing regular expression information, and a pointer to a C<marpaESLIFValueResultInt_t> value that will be reinjected into PCRE2, following L<PCRE2 callout|https://www.pcre.org/current/doc/html/pcre2callout.html> specification, i.e. (quote from PCRE2 documentation):
...
Note that in this case, if the negative value is lower than C<PCRE2_ERROR_INTERNAL_DUPMATCH>, ESLIF will emit a warning and change it to C<PCRE2_ERROR_CALLOUT>

=back

The C<marpaESLIFCalloutBlockp> is guaranteed to have the MARPAESLIF_VALUE_TYPE_TABLE type, with these keys:

=over

=item C<callout_number>

A C<MARPAESLIF_VALUE_TYPE_LONG> or C<MARPAESLIF_VALUE_TYPE_UNDEF> value. C<MARPAESLIF_VALUE_TYPE_LONG> is when the callout is in the form C<(?Cnumber)>, C<MARPAESLIF_VALUE_TYPE_UNDEF> when the callout is the form C<(?C"string")>.

=item C<callout_string>

A C<MARPAESLIF_VALUE_TYPE_ARRAY> or C<MARPAESLIF_VALUE_TYPE_UNDEF> value. C<MARPAESLIF_VALUE_TYPE_ARRAY> is when the callout is in the form C<(?C"string")>, C<MARPAESLIF_VALUE_TYPE_UNDEF> when the callout is the form C<(?Cnumber)>. C<MARPAESLIF_VALUE_TYPE_ARRAY> points to the inner string.

=item C<subject>

A C<MARPAESLIF_VALUE_TYPE_ARRAY> pointing to current subject bytes.

=item C<pattern>

A C<MARPAESLIF_VALUE_TYPE_ARRAY> pointing to current PCRE2 pattern bytes.

=item C<capture_top>

A C<MARPAESLIF_VALUE_TYPE_LONG> giving the maximum current capture

=item C<capture_last>

A C<MARPAESLIF_VALUE_TYPE_LONG> giving the most recently closed capture.

=item C<offset_vector>

A C<MARPAESLIF_VALUE_TYPE_ROW> of C<MARPAESLIF_VALUE_TYPE_LONG>s giving offsets within the subject.

=item C<mark>

A C<MARPAESLIF_VALUE_TYPE_LONG> or C<MARPAESLIF_VALUE_TYPE_UNDEF> giving current mark offset.

=item C<start_match>

A C<MARPAESLIF_VALUE_TYPE_LONG> giving current match start attempt offset.

=item C<current_position>

A C<MARPAESLIF_VALUE_TYPE_LONG> giving current subject offset.

=item C<next_item>

A C<MARPAESLIF_VALUE_TYPE_ARRAY> or C<MARPAESLIF_VALUE_TYPE_UNDEF> giving next item the pattern.

=item C<grammar_level>

A C<MARPAESLIF_VALUE_TYPE_INT> giving current grammar level.

=item C<symbol_id>

A C<MARPAESLIF_VALUE_TYPE_INT> giving the current symbol identifier within current grammar.
...
From there, any update to the new recognizer's stream or C<marpaESLIFRecognizerSharedp>'s stream are synchronous and shared. Running the new recognizer and the shared recognizer in parallel is not supported.
...
Child recognizer inherits a shallow copy of C<marpaESLIFRecognizerSharedp>'s context.
...
The only recognizer option that can be changed at runtime is the C<exhaustedb> flag. The later is important when the recognizer cannot accept any new data (i.e. parse is exhausted) but unprocessed data remains: if C<exhaustedb> is a false value, then an error is raised. Please see the exhaustion item in the NOTES section.
...
Any C<marpaESLIFReader_t> callback output will after the stream of both recognizers, as if they were both owning the internal buffers attached to the stream. In particular the input position of both recognizers are affected: they will I<move> in parallel. If you want C<marpaESLIFRecognizerSharedp> to not be affected, then you must use the C<marpaESLIFRecognizer_peekb> method.

It is illegal to have C<marpaESLIFRecognizerp> equal to C<marpaESLIFRecognizerSharedp>.

This method returns a true value in case of success, a false value otherwise.

=head3 C<marpaESLIFRecognizer_peekb>
...
The recognizer C<marpaESLIFRecognizerp> is instructed to share the stream with C<marpaESLIFRecognizerPeekedp>, though it is guaranteed that the internal stream buffer can only grow until it is unpeeked. This guarantees that C<marpaESLIFRecognizerPeekedp> is not affected by any C<marpaESLIFRecognizerp>'s reader callback. When C<marpaESLIFRecognizerPeekedp> is C<NULL>, peek is stopped. 

It is legal to have C<marpaESLIFRecognizerp> equal to C<marpaESLIFRecognizerPeekedp>, this mean that the user wants to "freeze" the stream, allowing only the internal buffers to grow.

It is the application responsibility to make sure that the lifetime of C<marpaESLIFRecognizerPeekedp> is <greater> than C<marpaESLIFRecognizerp>.
...
Calling C<marpaESLIFRecognizer_resumeb> is legal for any recognizer that already called <marpaESLIFRecognizer_scanb> or did at least one call to C<marpaESLIFRecognizer_alternativeb> or C<marpaESLIFRecognizer_alternative_readb>.
...
It is possible to inject symbols outside of the internal scanner. They are then are described using a C<marpaESLIFAlternative_t> structure, containing:

=over

=item names

The symbol name as per the grammar. Must be different than C<NULL>.
...
=head3 C<marpaESLIFRecognizer_alternative_completeb>

  short marpaESLIFRecognizer_alternative_completeb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp,
                                                   size_t lengthl);

Say to the recognizer pointed C<marpaESLIFRecognizerp> by that the current set of alternatives is complete, and that the stream should advance by C<length> bytes, where bytes is really the number of bytes in the internal recognizer buffer, i.e. an UTF-8 sequence of code points in case of a character-oriented stream. The reader callback may be called if necessary, in case the caller say to advance by more bytes than what is currently available. It is an error to specify more bytes than the recognizer can get (and eventually transform, in case of character-stream oriented buffer) from the reader. A a value of C<0> for the length is legal.

If a I<after> event is triggered, C<marpaESLIFRecognizer_name_last_pauseb> will show as many bytes as C<lengthl>. And because it is guaranteed that C<marpaESLIFRecognizer_name_last_pauseb> returns a pointer to a NUL-terminated buffer, the extreme case is that last pause can be an empty string "", which is legal when the alternative has been pushed using a user-defined value or it is correspond to a pseudo terminal.

This method can generate grammar and pause events.

Returns a true value on success, a false value on failure.

=head3 C<marpaESLIFRecognizer_alternative_readb>
...
=head3 C<marpaESLIFRecognizer_name_tryb>

  short marpaESLIFRecognizer_name_tryb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp,
                                       char *names,
                                       short *matchbp);

Try to match the symbol with name C<names> that must be non-C<NULL> against the input seen by the recognizer. Returns in C<matchbp>, when not C<NULL>, a true value if there is a match, a false value otherwise. This method generates no event.

This function should never fail, a false value indicates something fatal happened or the arguments are not correct.

=head3 C<marpaESLIFRecognizer_discardb>

  short marpaESLIFRecognizer_discardb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp, size_t *discardlp);

Applies :discard on the stream, returning in C<discardlp>, when not C<NULL>, a value that is > 0 if discard was successful.

The discarded data itself can be retreived I<only> if there is an associated completion event, e.g.:
...
=head3 C<marpaESLIFRecognizer_name_expectedb>

  short marpaESLIFRecognizer_name_expectedb(marpaESLIFRecognizer_t   *marpaESLIFRecognizerp,
                                            size_t                   *nNamelp,
                                            char                   ***namesArraypp);

For the current recognizer C<marpaESLIFRecognizerp>, get the list of I<names> expected. C<*nNamelp>, if not C<NULL>, is filled with the number of expected names, and C<*namesArraypp>, if not C<NULL>, will contain the array of NUL-terminated ASCII symbol names.
...
=head3 C<marpaESLIFRecognizer_isStartCompleteb>

  short marpaESLIFRecognizer_isStartCompleteb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp,
                                              short *completebp);

Returns in the value pointed by C<completebp> a true value if grammar reached start symbol completion, when it is not C<NULL>.
...
I<symbol before> event. This can happen only for lexemes or terminals that are predicted by the scanning. In the grammar this is declared with:
...
I<symbol before> event. This can happen only for lexemes or terminals after they are pushed as alternatives by the scanning. In the grammar this is declared with:
...
=head3 C<marpaESLIFRecognizer_progressb>
...
Asks for the raw version progress, and from C<starti> to C<endi> indices. The C<starti> and C<endi> indices are Earleme Set Ids, and obey the Perl language convention, i.e. a negative indice means it far back from the end. If not C<NULL>, returns in C<progresslp> the number of progress items, themselves stored in C<progresspp> when the later is not C<NULL>. A progress items is a C<marpaESLIFRecognizerProgress_t> structure composed of:
...
=head3 marpaESLIFRecognizer_latestEarleySetIdb>

  short marpaESLIFRecognizer_latestEarleySetIdb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp,
                                                int *latestEarleySetIdip);

When C<latestEarleySetIdip> is not C<NULL>, it contains the latest Earley Set Id.

Returns a true value on success, a false value otherwise.

=head3 marpaESLIFRecognizer_earlemeb

  short marpaESLIFRecognizer_earlemeb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp,
                                      int earleySetIdi,
                                      int *earlemeip);

When C<earlemeip> is not C<NULL>, it contains the earleme Id corresponding to the Earley Set Id C<earleySetIdi>. The later follows the Perl language convention, i.e. a negative indice means it far back from the end.
...
=head3 C<marpaESLIFRecognizer_errorb>

  short marpaESLIFRecognizer_errorb(marpaESLIFRecognizer_t  *marpaESLIFRecognizerp);

Generates an error report for the recognizer instance pointed by C<marpaESLIFRecognizerp>.
...
=head3 C<marpaESLIFRecognizer_name_last_pauseb>
...
=head3 C<marpaESLIFJSON_encode_newp>

  marpaESLIFGrammar_t *marpaESLIFJSON_encode_newp(marpaESLIF_t *marpaESLIFp, short strictb);

ESLIF embeds a JSON grammar encoder, where C<strictb> says it is strictly the grammar as per L<JSON Specification|http://json.org>. The non strict mode enables:

=over

=item Infinity and NaN

=back

=head3 C<marpaESLIFJSON_encodeb>
...
=head3 C<marpaESLIFJSON_decode_newp>

  marpaESLIFGrammar_t *marpaESLIFJSON_decode_newp(marpaESLIF_t *marpaESLIFp, short strictb);

ESLIF embeds a JSON grammar decoder, where C<strictb> says it is strictly the grammar as per L<JSON Specification|http://json.org>. The non strict mode enables:

=over

=item extras comma separators

=item Perl and C++ comments

=item Infinity and NaN

=item Leading zeroes in number

=item Case-insensitive for constants

=back

=head3 C<marpaESLIFJSON_decodeb>
...
Concerning numbers, since every host might want to have its own representation, every proposal callback that is not C<NULL> will be called. The proposal will be in the C<marpaESLIFValueResultp> pointer, guaranteed to not be C<NULL>, and the caller can safely modify it. The C<confidenceb> will be a true value if ESLIF is sure it represents correctly the number, a false value otherwise.

For infinity and NaN callbacks, when C<confidenceb> is a true value, C<marpaESLIFValueResultp> type is always C<MARPAESLIF_VALUE_TYPE_FLOAT>, else it is C<MARPAESLIF_VALUE_TYPE_UNDEF>.

For other numbers, C<confidenceb> is a true value C<marpaESLIFValueResultp> type is an integer type, or the double type and when the double value is exactly 0. Else this will be a double type, or C<MARPAESLIF_VALUE_TYPE_UNDEF>.

In general it is safe to leave untouched the C<marpaESLIFValueResultp> if C<confidenceb> is a true value, else it is recommended to change it.

Take care of the context in the callbacks:

=over

=item Decoder option callbacks

They are called in the context of a recognizer. So they are using C<marpaESLIFJSONDecodeOptionp->userDatavp> in their context.

=item Import callback

It is called in the context of a valuator. So they are using C<marpaESLIFValueOptionp->userDatavp> in their context.
...
=item ruleActionResolverp

If there a custom rule, this callback must not be C<NULL> and return a function pointer that will be used to perform valuation. This function must be return a true value otherwise valuation will stop.

See below.

=item symbolActionResolverp

If there a custom generic C<:default symbol-action>, this callback must not be C<NULL> and return a function pointer that will be used to perform symbol value transition. This function must be return a true value otherwise valuation will stop.
...
=head3 C<marpaESLIFValue_marpaESLIFValueResult_freeb>

    marpaESLIF_EXPORT short marpaESLIFValue_marpaESLIFValueResult_freeb(marpaESLIFValue_t *marpaESLIFValuep,
                                                                        marpaESLIFValueResult_t *marpaESLIFValueResultp,
                                                                        short deepb);

Handy method to be used when you use C<marpaESLIFValue_stack_forgetb>. If C<deepb> is a false value, only the first immediate pointer, if any, inside C<marpaESLIFValueResultp> will be freed when not shallow. If C<deepb> is a true value, loops over all inner marpaESLIFValueResult's and call for disposal of every non-shadowed element. This method never free C<marpaESLIFValueResultp> itself because it may be on the stack. It is user's responsibility to handle this case.


=head3 C<marpaESLIFRecognizer_marpaESLIFValueResult_freeb>

    marpaESLIF_EXPORT short marpaESLIFRecognizer_marpaESLIFValueResult_freeb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp,
                                                                             marpaESLIFValueResult_t *marpaESLIFValueResultp,
                                                                             short deepb);

Handy method equivalent to C<marpaESLIFValue_marpaESLIFValueResult_freeb> but in a recognizer context.
...
For strings and regular expressions, the C<modifiers>, when not C<NULL>, must be compliant with the grammar specification for strings and regular expressions, i.e.:
...
For meta symbols, also they can belong to any C<marpaESLIFGrammarp>, only the lexemes (i.e. those that are defined in a sub-grammar) can be tried with C<marpaESLIFSymbol_tryb> and C<marpaESLIFRecognizer_symbol_tryb>.

For strings and regular expressions, the C<stringp> argument is without the delimiters, e:g:
...
=item Grammar exhaustion

The grammar is truely exhausted, i.e. it cannot accept any alternative.

=item Start completion and nothing is accepted
...
Automatic imports rely on the C<importerp> option in their respective constructors:
...
These callbacks must return function pointers that have this signature (function names here are just examples):
  ...
  short userDefinedRecognizerImporter(marpaESLIFRecognizer_t *marpaESLIFRecognizerp, void *userDatavp, marpaESLIFValueResult_t *marpaESLIFValueResultp);
  short userDefinedValueImporter(marpaESLIFValue_t *marpaESLIFValuep, void *userDatavp, marpaESLIFValueResult_t *marpaESLIFValueResultp);

i.e. they are called with their respective namespace and user-defined C<userDatavp> that is set in their respective constructors.
...
that are just proxies to C<userDefinedRecognizerImporter> and C<userDefinedValueImporter>, respectively.

An importer is a stack safe recursive mechanism that establishes the following data contract with the implementation:

=over

=item C<MARPAESLIF_VALUE_TYPE_ROW> and C<MARPAESLIF_VALUE_TYPE_TABLE> marpaESLIFValueResult's types are flatified:

=over

=item C<MARPAESLIF_VALUE_TYPE_ROW>

=over

=item

All members are imported first in order. In other words, the row has C<marpaESLIFValueResultp->u.r.sizel> items, so:
...
Then the marpaESLIFValueResult itself, of type C<MARPAESLIF_VALUE_TYPE_ROW> is imported:

  marpaESLIFValueResultp /* Type is MARPAESLIF_VALUE_TYPE_ROW */

=back

=item C<MARPAESLIF_VALUE_TYPE_TABLE>

=over

=item

All key/value members are imported first in order. In other words, the table has C<marpaESLIFValueResultp->u.t.sizel> items, so:
...
Then the marpaESLIFValueResult itself, of type C<MARPAESLIF_VALUE_TYPE_TABLE> is imported:
...
=item Other marpaESLIFValueResult types are imported as-is, because they are just proxies to native C data types, they are not containers.

=back

This mean that the importer callback should maintain an internal stack of its transformed marpaESLIFValueResult_t items everytime it is called and:

=over

=item

push to its stack anything that is not a C<MARPAESLIF_VALUE_TYPE_ROW> nor a C<MARPAESLIF_VALUE_TYPE_TABLE>

=item

pop from its stack C<u.r.sizel> elements when it receives a C<MARPAESLIF_VALUE_TYPE_ROW> callback, transforming them to what is an array for the host language

=item

pop from its stack C<u.t.sizel * 2> elements when it receives a C<MARPAESLIF_VALUE_TYPE_TABLE> callback, transforming them to what is a hash for the host language
...
[1] Context dependant, can be a lua specific value, injected by hand during recognizer phase for example, falling back to a LUA_TUSERDATA
...
[5] java.lang.Byte or java.lang.Short or java.lang.Integer or java.lang.Long or java.math.BigInteger, depends on C<CHAR_BIT>
...
A readonly C<SV*> that is a C<PV*> pointing directly to the memory region.

=item Lua language

A L<xstring-lua|https://github.com/chipdude/xstring-lua>-like object, where only four methods exists:
...
Returns a new I<xtring> instance mapped to the given subregion.
...
=head1 Strings and Array lifetime

Here are the rules for strings and arrays lifetime:

=over

=item C<marpaESLIF_versionb>

The ASCII string returned in C<*versionsp> is persistent ESLIF is freed. You can access it directly in host space, though any change will result to undefined behaviour.

=item C<marpaESLIFAction> type

ESLIF guarantees that all string pointers in C<marpaESLIFAction> are persistent until the associated grammar is freed. You can access them directly in host space, though any change to them will result to undefined behaviour.

=item C<marpaESLIFGrammar_grammar_by_levelb>

ESLIF guarantees that the string returned in C<*descpp> is persistent until the associated grammar is freed. You can access it directly in host space, though any change will result to undefined behaviour.

=item C<marpaESLIFGrammar_grammarproperty_currentb> 

Any action and string returned in persistent until the associated grammar is freed. Any change to them will result to undefined behaviour.

=item C<marpaESLIFGrammar_ruleproperty_currentb> and C<marpaESLIFGrammar_ruleproperty_by_levelb>

Any action and string returned in persistent until the associated grammar is freed. Any change to them will result to undefined behaviour.

=item C<marpaESLIFGrammar_ruledisplayform_currentb> and C<marpaESLIFGrammar_ruledisplayform_by_levelb>

The I<ASCII> string returned in C<*ruledisplaysp> is persistent until the associated grammar is freed. Any change to it will result to undefined behaviour.

=item C<marpaESLIFGrammar_symbolproperty_currentb> and C<marpaESLIFGrammar_symbolproperty_by_levelb>

Any action and string returned in persistent until the associated grammar is freed. Any change to them will result to undefined behaviour.

=item C<marpaESLIFGrammar_symboldisplayform_currentb> and C<marpaESLIFGrammar_symboldisplayform_by_levelb>

The I<ASCII> string returned in C<*symboldisplaysp> is persistent until the associated grammar is freed. Any change to it will result to undefined behaviour.

=item C<marpaESLIFGrammar_ruleshowform_currentb> and C<marpaESLIFGrammar_ruleshowform_by_levelb>

The I<ASCII> string returned in C<*ruleshowsp> is persistent until the associated grammar is freed. Any change to it will result to undefined behaviour.

=item C<marpaESLIFGrammar_grammarshowform_currentb> and C<marpaESLIFGrammar_grammarshowform_by_levelb>

The I<ASCII> string returned in C<*grammarshowsp> is persistent until the associated grammar is freed. Any change to it will result to undefined behaviour.

=item C<marpaESLIFGrammar_grammarshowscriptb>

The I<ASCII> string returned in C<*grammarscriptsp> is persistent until the associated grammar is freed. Any change to it will result to undefined behaviour.

=item C<marpaESLIFValue_contextb>

The I<ASCII> strings returned in C<*symbolsp> and C<*rulesp> are persistent until the associated grammar is freed. Any change to them will result to undefined behaviour.

=item Recognizer callbacks

All string and arrays are valid during the callback and are guaranteed to not change C<if> ESLIF recognizer state is not affected by any user call. Calls that C<may> affect ESLIF recognizer state are:

=over

=item C<marpaESLIFRecognizer_scanb>

=item C<marpaESLIFRecognizer_resumeb>

=item C<marpaESLIFRecognizer_alternative_readb>

=item C<marpaESLIFRecognizer_alternative_completeb>

=item C<marpaESLIFRecognizer_readb>

=back

Callbacks that give direct pointers to memory are:

=over

=item C<marpaESLIFRecognizerIfCallback_t>

=item C<marpaESLIFRecognizerEventCallback_t>

=item C<marpaESLIFRecognizerRegexCallback_t>

=item C<marpaESLIFRecognizerImport_t>

=back

The memory pointers if the above methods are I<always> of type array, ESLIF bindings then guarantees that:

=over

=item Perl language

A read-only C<SV*> is used to access the memory content.

=item Lua language

A implementation very closed to L<xstring-lua|https://github.com/chipdude/xstring-lua> is used to access the memory content. Any attempt to change memory will result in unpredicted behaviour.

=item Java language

A C<DirectByteBuffer> is used to access the memory content. Any attempt to change memory will result in unpredicted behaviour.
