...
What is this C<nullableb> flag!? Well, it means what it means: sometimes a rule is nullable, for example the empty rule:
  ...
  nullableRule ::= action => my_nullable_action;

or a sequence rule:

  nullableRule ::= item* action => my_nullable_action

Then, and only in these cases, the C<nullableb> flag is a true value, C<arg0i> and C<argni> will be both C<-1>, e.g. (with C<my_nullable_actionp> being the rule resolver result for action C<"my_nullable_action">):

  my_nullable_actionp(userDatavp, marpaESLIFValuep, -1 /* arg0i */, -1 /* argni */, 0 /* resulti */, 1 /* nullableb */)
