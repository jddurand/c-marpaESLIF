...
A user-defined function pointer that, if not C<NULL>, is guaranteed to be called as soon as ESLIF do not need anymore what the reader returned.
...
For meta symbols, also they can belong to any C<marpaESLIFGrammarp>, only the grammar terminals can be tried with C<marpaESLIFSymbol_tryb> and C<marpaESLIFRecognizer_symbol_tryb>.
...
Please note that C<marpaESLIFRecognizer_symbol_tryb()> will use the symbol's importer, if any, not the recognizer's importer.
...
The flag C<haveUndefb> is always:
...
=item C<MARPAESLIF_VALUE_TYPE_UNDEF>

=item C<MARPAESLIF_VALUE_TYPE_ROW>

and one of the elements is of type C<MARPAESLIF_VALUE_TYPE_UNDEF>

=item C<MARPAESLIF_VALUE_TYPE_TABLE>

and one of the elements (that can be the key or the value) is of type C<MARPAESLIF_VALUE_TYPE_UNDEF>.
...
The ASCII string returned in C<*versionsp> is persistent until ESLIF is freed.

=item C<marpaESLIFAction> type

ESLIF guarantees that all string pointers in C<marpaESLIFAction> are persistent until the associated grammar is freed.

=item C<marpaESLIFGrammar_grammar_by_levelb>

ESLIF guarantees that the string returned in C<*descpp> is persistent until the associated grammar is freed.
...
The I<ASCII> string returned in C<*ruledisplaysp> is persistent until the associated grammar is freed.
...
The I<ASCII> string returned in C<*symboldisplaysp> is persistent until the associated grammar is freed.

=item C<marpaESLIFGrammar_ruleshowform_currentb> and C<marpaESLIFGrammar_ruleshowform_by_levelb>

The I<ASCII> string returned in C<*ruleshowsp> is persistent until the associated grammar is freed.

=item C<marpaESLIFGrammar_grammarshowform_currentb> and C<marpaESLIFGrammar_grammarshowform_by_levelb>

The I<ASCII> string returned in C<*grammarshowsp> is persistent until the associated grammar is freed.

=item C<marpaESLIFGrammar_grammarshowscriptb>

The I<ASCII> string returned in C<*grammarscriptsp> is persistent until the associated grammar is freed.

=item C<marpaESLIFValue_contextb>

The I<ASCII> strings returned in C<*symbolsp> and C<*rulesp> are persistent until the associated grammar is freed.
...
=item C<marpaESLIFRecognizer_inputb()>

=item C<marpaESLIFRecognizerIfCallback_t> structures in if callbacks

=item C<marpaESLIFRecognizerEventCallback_t> structures in event callbacks

=item C<marpaESLIFRecognizerRegexCallback_t> structures in regular expression callbacks

=item C<marpaESLIFRecognizerImport_t()>

=item C<marpaESLIFRecognizer_symbol_tryb()>

=back

The memory pointers in the above methods are I<always> of type C<MARPAESLIF_VALUE_TYPE_ARRAY>, ESLIF bindings then guarantees that:
...
A read-only <external string> implementation, very closed to L<xstring-lua|https://github.com/chipdude/xstring-lua> is used to access the memory content. Take care these are I<not> true strings from Lua point of view. The following methods exist on such object:
...
Returns a true Lua string. This method will trigger a I<copy into Lua>.
