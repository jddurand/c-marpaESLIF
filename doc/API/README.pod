...
Please note that user-defined lexemes can trigger a I<lexeme after> event, though the information retreived with C<marpaESLIFRecognizer_lexeme_last_pauseb> will never show anything else than correspond to real input stream.
...
If a I<lexeme after> event is triggered, C<marpaESLIFRecognizer_lexeme_last_pauseb> will show as many bytes as C<lengthl>. And because it is guaranteed that C<marpaESLIFRecognizer_lexeme_last_pauseb> returns a pointer to NUL-terminated buffer, the extreme case is that last pause can be an empty string "".
