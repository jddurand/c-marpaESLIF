...
Value type is C<marpaESLIFValueResultString_t>, typedef'ed to: C<struct { unsigned char *p; short shallowb; size_t sizel; char *encodingasciis; }>.
...
Because this is representing a lexeme, which is always of type ARRAY in the value stack (i.e. a pointer to bytes together with an associated length), here is how the rule and symbol callbacks can distinguish between user-defined alternatives and those coming from the internal recognition: the pointer is non-C<NULL>, but the length is C<0> and the context is C<0> in a rule callback.

ESLIF guarantees that an alternative cannot point to another alternative.

Examples on how to detect user-defined alternatives follows:
  ...
  short myRuleCallback(void              *userDatavp,
    ...
    if (! nullableb) {
      ...
      if (marpaESLIFValueResultp->type == MARPAESLIF_VALUE_TYPE_ARRAY) {
        if ((! marpaESLIFValueResultp->contexti) && (marpaESLIFValueResultp->u.p != NULL) && (marpaESLIFValueResultp->sizel <= 0)) {
          /* This is a user-defined lexeme, i.e. an alternative, referenced by u.p */
          marpaESLIFValueResultp = (marpaESLIFValueResultp_t *) marpaESLIFValueResultp->u.p;
...
=head3 C<marpaESLIFValue_newp>
  ...
  typedef struct marpaESLIFValueOption {
    ...
    marpaESLIFValueFreeActionResolver_t   freeActionResolverp;   /* Returns pointer to free action */
    marpaESLIFValueResultTransform_t     *transformerp;          /* Asks end-user to process a value */
...
=item 5. Put user data at stack indice C<0> and context C<0x78>
...
ESLIF never keeps in memory the I<final> result. Before freeing its internal stack, it will ask the user to do a <transformation> using C<transformerp>. If a transformer is C<NULL> for a given value type, it will raise an error.

The goal of the transformer is to get into user's space a managable equivalent to an ESLIF value. For example, the provided bindings are doing this transformation:

  |------------------------------------------------------------------------------------------|
  | value type                    | perl (XS)            | java       | lua (C)              |
  |------------------------------------------------------------------------------------------|
  | undefined                     | &PL_sv_undef         | null       | LUA_TNIL             |
  | marpaESLIFValueResultChar_t   | PV                   | Character  | LUA_TSTRING          |
  | marpaESLIFValueResultShort_t  | IV                   | Short      | LUA_TNUMBER          |
  | marpaESLIFValueResultInt_t    | IV                   | Integer    | LUA_TNUMBER          |
  | marpaESLIFValueResultLong_t   | IV                   | Long       | LUA_TNUMBER          |
  | marpaESLIFValueResultFloat_t  | NV                   | Float      | LUA_TNUMBER          |
  | marpaESLIFValueResultDouble_t | NV                   | Double     | LUA_TNUMBER          |
  | marpaESLIFValueResultPtr_t    | IV                   | Long       | LUA_TLIGHTUSERDATA   |
  | marpaESLIFValueResultArray_t  | PV                   | byte[]     | LUA_TSTRING          |
  | marpaESLIFValueResultBool_t   | &PL_sv_no/&PL_sv_yes | Boolean    | LUA_TBOOLEAN         |
  | marpaESLIFValueResultString_t | UTF-8'ed PV          | String     | UTF-8'ed LUA_TSTRING |
  |------------------------------------------------------------------------------------------|

The transformer callbacks are really helpers that do not depend on valuation context, i.e. it is perfectly legal to call them outside valuation.
...
=head3 C<marpaESLIFValue_transformb>

  short marpaESLIFValue_stack_get_transformb(marpaESLIFValue_t *marpaESLIFValuep, int indicei);

During valuation, the caller can call for transformation of value at stack C<indicei>.

Returns a true value on success, a false value otherwise.

=head3 C<marpaESLIFValue_charconvb>
...
Helper function that is doing character translation of a buffer starting at C<srcs> and of size C<srcl> bytes. Initial encoding is C<toEncodings>, if C<NULL> it will be guessed. Final encoding is C<toEncodings> and must be set. Final number of bytes is stored if C<dstlp> if it is not NULL.

Please note that the transcoded result I<may> contain a BOM, it is up to the caller to check that.

Returns a pointer to the transcoded string on success, C<NULL> on failure.
