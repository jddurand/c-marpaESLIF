...
Try to match the lexeme with name C<lexemes> that must be non-C<NULL>. Returns in C<matchbp>, when not C<NULL>, a true value if there is a match, a false value otherwise. This method generates no event.
