...
All the objects are re-entrant but only I<marpaESLIF_t> and <marpaESLIFSymbol_t> are thread-safe. Therefore it is safe to use a <marpaESLIF_t> or a <marpESLIFSymbol_t> object as a singleton, other objects should be cloned if the same function is to be called on them from multiple threads, unless the user can synchronize their access.
