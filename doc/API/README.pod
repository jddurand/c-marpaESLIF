...
A pointer to a C<genericLogger_t> instance. If it is C<NULL>, then C<marpaESLIF> instance, neither any of the other namespaces, will do logging. Otherwise, all will inherit this logger object and use it.
...
The C<marpaESLIF> destructor, with parameter C<marpaESLIFp> being an instance previously created. No-op if C<marpaESLIFp> is C<NULL>.
...
Return the main ESLIF object instance from a C<marpaESLIFGrammarp> ESLIFGrammar instance, or C<NULL> is failure. It never returns C<NULL> if C<marpaESLIFGrammarp> is not C<NULL>.
...
=head3 C<marpaESLIFRecognizer_lexeme_alternative_lengthb>

  short marpaESLIFRecognizer_lexeme_alternative_lengthb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp,
                                                        size_t alternativeLengthl);

When a recognizer is paused, user have the possibility to inject lexemes. Though the marpaESLIF does I<not> support injection of lexemes that would be of different size in the input stream: all lexemes must be of the same size C<alternativeLengthl>, which is in I<byte> unit. At every pause of the scanning, this method can be called once, and it is illegal to give a length higher than the number of remaining bytes in marpaESLIF internal buffer (see the C<marpaESLIFRecognizer_readb> for injecting more data, and C<marpaESLIFRecognizer_inputv> for get information of the internal buffers).
...
Say to the recognizer pointed by C<marpaESLIFRecognizerp> that the lexeme C<lexemes> (which is an ASCII NUL terminated string) is an alternative. Please note that, as per the grammar, a symbol name is always composed of ASCII characters only.
...
Say to the recognizer pointed C<marpaESLIFRecognizerp> by that the current set of lexeme alternatives is complete: these are the ones pushed with C<marpaESLIFRecognizer_alternativeb> method, and they all take the same number of bytes in the input stream declared with C<marpaESLIFRecognizer_alternative_lengthb>.
...
A short-hand version of lexeme management that is pushing a lexeme C<lexemes> (which is ASCII NUL terminated string), of size C<lexemel> bytes in the input stream, as a single alternative and call for alternative completion.
...
=head3 C<marpaESLIFRecognizer_eventb>
  ...
  void marpaESLIFRecognizer_eventb(marpaESLIFRecognizer_t  *marpaESLIFRecognizerp,
                                   size_t                  *eventArraylp,
...
Send to the logger configured at the marpaESLIF instanciation step, the current progress of parsing of the C<marpaESLIFRecognizerp> recognizer instance. Logging is do at level C<logleveli> (c.f. L<genericLogger|https://github.com/jddurand/c-genericLogger> documentation), and from C<starti> to C<endi> indiced. The C<starti> and C<endi> indices obey the Perl language convention, i.e. a negative indice means it far back from the end. The format of the strings generated is totally under the L<marpaWrapper|https://github.com/jddurand/c-marpaWrapper> library.

Returns a true value on success, a false value on failure.

=head3 C<marpaESLIFRecognizer_inputv>

  void marpaESLIFRecognizer_inputv(marpaESLIFRecognizer_t  *marpaESLIFRecognizerp,
                                   char                   **inputsp,
                                   size_t                  *inputlp,
                                   short                   *eofbp);
...
If not C<NULL>, will contain the current I<end of file> flag.

=back

These pointers are intentionnaly not having a I<const> keyword, though it is obvious that any attempt to modify C<*inputsp> or C<*inputlp> is likely to make the process crash. Modifying C<*eofbp> is less dangerous, in the sense that forcing it for a false value will simply force the recognizer to call the reader callback.

=head3 C<marpaESLIFRecognizer_pausev>

  void marpaESLIFRecognizer_pausev(marpaESLIFRecognizer_t  *marpaESLIFRecognizerp,
                                   char                   **pausesp,
                                   size_t                  *pauselp,
                                   short                   *eofbp);

For the recognizer instance pointed by C<marpaESLIFRecognizerp>, returns a pointer to the last portion of input stream that generated a I<lexeme pause> event in the value pointed by C<pausesp> if it is not NULL, and the length in the value pointed by C<pauselp> if it is not NULL. For convenience if the last pause is not NULL, the buffer holding the pause information is always NUL-terminated, even if the C<\0> NUL character is I<not> in the returned length. For convenence as well, the current I<end of file> flag is returned in the value pointed by C<eofbp> if it is not C<NULL>.
...
Forces a call to the reader callback. All parameters have the semantic as in the method C<marpaESLIFRecognizer_inputv>.
...
The C<marpaESLIFRecognizer> destructor, with parameter C<marpaESLIFRecognizerp> being an instance previously created. No-op if C<marpaESLIFRecognizerp> is C<NULL>.
...
and nothing impose the action resolvers to always return the same function pointer.

The value result itself is also not under marpaESLIF control, though the later requires the user to manipulate a I<stack>, and to assign to every entry in the stack a I<value type>, and a I<value context>. So there is a notion of I<user stack>, which start at indice number C<0>, and whose length is determined by the parse tree value location. The context of every I<value> in the stack is nothing else but a free number that should describe what it is, and has a meaning only for the end-user. It is important to set a correct context when assigning values to the stack, because this is how the I<memory free> function pointer, another function leaving in user space, will know how to react. For example a rule like this:

  my_rule ::= I J 'string terminal' action => my_action

could be called with output stack indice C<0> and input stack indices C<0> for C<I>, C<1> for C<J>, and C<2> for C<K>. It the end user that knows what it is about. Suppose that types of data types would be, in order, a pointer (C<PTR> below) with context value C<17>, an integer (C<INT> below) with context value C<216>, and a short (C<SHORT> below) with context value C<43>, and that user decide that the result is a long with context C<20>, then a representation of the stack is:

  Stack indice:          0           1         2      ==>         0
          Type:        PTR         INT     SHORT               LONG
       Context:        17          216        43                 20
...
This mean that, at any time, a stack indice can be replaced by another. This is a decision of the parse tree engine, based on rule definitions. And since in our example stack at indice C<0> is a pointer, marpaESLIF will I<automatically> call for I<memory free> action on this indice, before replacing it by the C<my_rule> result. The I<free action> is set at the grammar level:

  :default ::= free-action => my_free_action

=back

Full history of rule action is then:

=over

=item 1 - Call the action resolver for C<"my_action">

The function call is equivalent to: C<ruleActionResolverp(userDatavp, marpaESLIFValuep, "my_action")>, suppose that it returned a function pointer to C<my_actionp>, a function that exist in user-space.

=item 2 - Call C<my_actionp> with input parameter range C<[0-2]> and output parameter C<0>

The function call is is C<my_actionp(userDatavp, marpaESLIFValuep, 0 /* arg0i */, 2 /* argni */, 0 /* resulti */, 0 /* nullableb */)>. What is this C<nullableb> flag then !? Well, it means what it means: sometimes a rule is nullable, for example the empty rule:
...
Then, and only in these cases, the C<nullableb> flag is a true value, C<arg0i> and C<argni> will be both C<-1>.
...
The function call is equivalent to: C<freeActionResolverp(userDatavp, marpaESLIFValuep, "my_free_action")>, suppose that it returned a function pointer to C<my_free_actionp>, a function that exist in user-space.

=item 4 - Call C<my_free_actionp> with input C<0> and context C<17>

  my_free_actionp(userDatabp, 17, p, l)

where p is a pointer to the memory holding previous value at stack indice C<0>. C<l> is zero in this case, we explain later when it can be different than zero. C<17> is the context, so that the function pointer C<my_free_actionp> knows what we are talking about.
