...
Put in the structure pointed by C<marpaESLIFGrammarDefaultsp>, if it is not C<NULL>, the current grammar defaults, where a C<NULL> value for a default means it is not set in the grammar.
