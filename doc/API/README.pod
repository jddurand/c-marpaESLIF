...
Any application must start by instanciating a main ESLIF. This object is thread-safe, re-entrant, and can be reused as many times as needed (in contrary to grammars that may be derived from it, see ESLIFGrammar section below). The recommended usage is to create an ESLIF instance once for the entire process lifetime.
...
This method is returning a C<marpaESLIF> object, or C<NULL> in case of failure, using C<marpaESLIFOptionp> that is an eventual pointer to a C<marpaESLIFOption_t> structure, or C<NULL>. The C<marpaESLIFOption_t> structure contains:

=over

=item C<genericLoggerp>

A pointer to a C<genericLogger_t> instance. If it is C<NULL>, then ESLIF will never log. Otherwise, all namespaces will inherit this logger object and use it.
...
An I<ESLIFGrammar> is sparse array of grammars, each identified by a I<level>, that is the indice in this array. Most of the APIs in this section have two form, one for the I<current> grammar, one for a grammar at a wanted I<indice> or I<description> (a I<description> is a string identifying a level, this can be set when definition the grammar).

The default current grammar is always at level C<0>, that must exist. Level <0> is naturally considered like the I<top level> grammar. Nevertheless it is possible to instanciate a parsing at a I<different> level, in which case current grammar in derived namespaces like the recognizer or the valuation, is the level you asked for.
...
Returns the ESLIF parent object instance from a C<marpaESLIFGrammarp>, or C<NULL> if failure. It never returns C<NULL> if C<marpaESLIFGrammarp> is not C<NULL>.
...
Puts the number of sub-grammars in the value pointed by C<ngrammarip>, which can be C<NULL>. The number of grammars is the highest level plus one, and it is B<allowed> to have undefined grammars at any indice but level 0.
...
Puts in the structure pointed by C<marpaESLIFGrammarDefaultsp>, if it is not C<NULL>, the current grammar defaults, where a C<NULL> value for a default means it is not set in the grammar.
...
Put in the structure pointed by C<marpaESLIFGrammarDefaultsp>, if it is not C<NULL>, the grammar at level C<leveli> and/or description C<descp> defaults, where a C<NULL> value for a default means it is not set in the grammar.
...
Set all grammar default using the structure pointed by C<marpaESLIFGrammarDefaultsp>, if it is not C<NULL>. A C<NULL> value for a default has the same effect as if original grammar never had the associated keyword.
...
Put in the structure pointed by C<marpaESLIFGrammarDefaultsp>, if it is not C<NULL>, the grammar defaults at level C<leveli> and/or description C<descp>, where a C<NULL> value for a default means it is not set in the grammar.
