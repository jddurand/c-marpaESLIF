...
Tries to parse the I<ASCII> string pointed by C<s> of size C<sizel> bytes (if C<sizel> is 0, ESLIF will compute automatically the length) as a JSON relaxed number, i.e. with possible leading C<+> and zeroes. Returns in C<marpaESLIFValueResultp>, when not C<NULL>, the result, and in C<confidencebp>, when not C<NULL>, a true number if ESLIF is sure.
...
Deprecated, please refer C<marpaESLIFRecognizer_completeb>.
