...
Say to the recognizer pointed by C<marpaESLIFRecognizerp> that the lexeme C<symbols> (which is an ASCII NUL terminated string) is an alternative. Please note that, as per the grammar, a symbol name is always composed of ASCII characters only.
...
A short-hand version of lexeme management that is pushing a lexeme C<lexemes> of size C<lexemel> bytes as a single alternative and call for the alternatives completion.
