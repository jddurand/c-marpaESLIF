...
A recognizer is allowed to perform a match try on any symbol that is a terminal, including those that are not part of the current grammar:
  ...
  short                        marpaESLIF_symbol_tryb(marpaESLIF_t *marpaESLIFp,
...
All the objects are re-entrant but only I<marpaESLIF_t> and I<marpaESLIFSymbol_t> are thread-safe. Therefore it is safe to use a <marpaESLIF_t> or a I<marpESLIFSymbol_t> object as a singleton. I<marpaESLIFGrammar_t> objects should be cloned under thread creation. I<marpaESLIFRecognizer_t> and I<marpaESLIFValue_t> methods should be externally synchronized.

=head1 REFERENCE COUNT HELPERS

  short marpaESLIFGrammar_ref_incb(marpaESLIFGrammar_t *marpaESLIFGrammarp, int *refip);
  short marpaESLIFGrammar_ref_decb(marpaESLIFGrammar_t *marpaESLIFGrammarp, int *refip);
  short marpaESLIFGrammar_refb(marpaESLIFGrammar_t *marpaESLIFGrammarp, int *refip);

  short marpaESLIFRecognizer_ref_incb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp, int *refip);
  short marpaESLIFRecognizer_ref_decb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp, int *refip);
  short marpaESLIFRecognizer_refb(marpaESLIFRecognizer_t *marpaESLIFRecognizerp, int *refip);

  short marpaESLIFValue_ref_incb(marpaESLIFValue_t *marpaESLIFValuep, int *refip);
  short marpaESLIFValue_ref_decb(marpaESLIFValue_t *marpaESLIFValuep, int *refip);
  short marpaESLIFValue_refb(marpaESLIFValue_t *marpaESLIFValuep, int *refip);

All the methods above are reference count helpers for objects that are not thread-safe. there is no other logic on the reference count but initializing it to C<1> at object creation.
These reference count helpers always return the current reference count in C<refip> if it is not C<NULL> and the call succeeds, i.e. returns a true value. The failure can only happen in there is an invalid argument of if the reference count has an internal turnaround. The aller is responsible to synhronize the access on these methods.
