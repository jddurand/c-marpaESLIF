...
From there, any update to the new recognizer's stream or C<marpaESLIFRecognizerSharedp>'s stream are synchronous and shared.
...
A recognizer can be instructed to share the stream with another C<marpaESLIFRecognizerSharedp> at any time. When C<marpaESLIFRecognizerSharedp> is C<NULL>, sharing is stopped.
...
The input stream callback will use C<marpaESLIFRecognizerSharedp> settings if the later is not C<NULL>, the original C<marpaESLIFRecognizerp> otherwise.
