...
Say to the recognizer pointed by C<marpaESLIFRecognizerp> that symbol C<symbols> (which is an ASCII NUL terminated string) is an alternative. Please note that, as per the grammar, a symbol name is always composed only ASCII characters only.
