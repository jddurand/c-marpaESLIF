...
=item All members are imported first in order

In other words, the row has C<marpaESLIFValueResultp->u.r.sizel> items, so:
...
=item Then the marpaESLIFValueResult itself, of type C<MARPAESLIF_VALUE_TYPE_ROW> is imported:
...
=item All key/value members are imported first in order

In other words, the table has C<marpaESLIFValueResultp->u.t.sizel> items, so:

  &(marpaESLIFValueResultp->u.t.p[0].key)
  &(marpaESLIFValueResultp->u.t.p[0].value)
  ...
  &(marpaESLIFValueResultp->u.r.p[marpaESLIFValueResultp->u.r.sizel-1].key)
  &(marpaESLIFValueResultp->u.r.p[marpaESLIFValueResultp->u.r.sizel-1].value)

are imported.

=item Then the marpaESLIFValueResult itself, of type C<MARPAESLIF_VALUE_TYPE_TABLE> is imported:
...
=item push to its stack anything that is not a C<MARPAESLIF_VALUE_TYPE_ROW> nor a C<MARPAESLIF_VALUE_TYPE_TABLE>

=item pop from its stack C<marpaESLIFValueResultp->u.r.sizel> elements when it receives a C<MARPAESLIF_VALUE_TYPE_ROW> callback, transforming them to what is an array for the host language

=item pop from its stack C<marpaESLIFValueResultp->u.t.sizel * 2> elements when it receives a C<MARPAESLIF_VALUE_TYPE_TABLE> callback, transforming them to what is a hash for the host language

=item pop from its stack any other type, transforming it to something meaningful in the host language
...
=item [1] Context dependant, can be a lua specific value, injected by hand during recognizer phase for example, falling back to a LUA_TUSERDATA

=item [2] Encoding information is attached to internal string. Since strings in Lua are unique, one string can have only one encoding.

=item [3] A special niled table that indexed by number

=item [4] A special niled table indexed by strings

=item [5] java.lang.Byte or java.lang.Short or java.lang.Integer or java.lang.Long or java.math.BigInteger, depends on C<CHAR_BIT>

=item [6] java.lang.Float (includes +/-Inf and Nan) or java.lang.Double or java.math.BigDecimal, depends on the C native data size

=item [7] java.lang.Float or java.lang.Double or java.math.BigDecimal, depends on the C native data size

=item [8] java.lang.Float or java.lang.Double or java.math.BigDecimal

=item [9] java.lang.Object, if a lua specific value, injected by hand during recognizer phase for example, falling back to java.lang.Long

=item [10] java.lang.Float or java.lang.Double or java.math.BigDecimal

=item [11] java.lang.Byte or java.lang.Short or java.lang.Integer or java.lang.Long or java.math.BigInteger

=item [12] IV or Math::BigInt

=item [13] SV*, if be a perl specific value, injected by hand during recognizer phase for example, falling back PTR2IV()

=item [14] $MarpaX::ESLIF::true or $MarpaX::ESLIF::false

=item [15] PV* or MarpaX::ESLIF::String instance

=item [16] NV or Math::BigFloat

=item [17] IV or Math::BigInt
