...
where C<p> is a pointer to a copy of input stream C<'string terminal'>, of size C<l>, asking for the result to be pushed at stack indice number C<2>. It is the user that decided that it will put a short value at indice 2.

=item You talk about value types in the stack, but I<how> can this be set?

marpaESLIF provide stack manipulation functions, and it is simply impossible for the end-user to get or set something for the stack without using these functions. Stack itself is maintained inside marpaESLIF. Please see all C<marpaESLIFValue_stack_xxx> stack function documented later.

=item Stack number indice C<0> is in I<both input and output> indices

This mean that, at any time, a stack indice can be replaced by another. This is a decision of the parse tree engine, based on rule definitions. And since stack at indice C<0> is a pointer, marpaESLIF will I<automatically> call for I<memory free> action on this indice, before replacing it by the C<my_rule> result. The I<free action> set set at the grammar level:
...
=item 1 - Call the action resolver for C<"my_action">, the later returns function pointer C<my_actionp>

=item 2 - Call C<my_actionp> with input parameter range C<[0-2]> and output parameter C<0>

User function C<my_actionp> decided to return a value of type I<long>.

=item 3 - Call the free resolver for C<"my_free_action"> the later returns function pointer C<my_free_actionp>
