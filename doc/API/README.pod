...
=head3 C<marpaESLIFRecognizer_newp>
  ...
  typedef struct marpaESLIFRecognizerOption {
    ...
    size_t               bufsizl;                     /* Minimum stream buffer size. Recommended: 0 (== system default) */
...
Support of newline count. This is used when the parser fails to produce ab accurate report with line and number information. Works at the granularity of data chunk, and is meaningless if data chunk is not declared as a character stream.
