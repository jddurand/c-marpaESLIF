...
is declaring no lexeme: both C<[\s]+>, C</[a-z]+/> and C<'X'> are I<explicit> terminals in the same grammar level. There is no associated name, and the user will not be able to say "I want an alternative being C<'X'>". On the other hand:
