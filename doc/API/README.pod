...
If set, the proposal callbacks will be called with a proposal, that the caller can safely modify on the stack. The proposal may be of type C<MARPAESLIF_VALUE_TYPE_UNDEF> only when the underlying system do not support or failed to parse:
