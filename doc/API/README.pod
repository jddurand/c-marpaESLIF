...
=head3 C<marpaESLIF_optionb>

  short marpaESLIF_optionb(marpaESLIF_t        *marpaESLIFp,
                           marpaESLIFOption_t **marpaESLIFOptionpp);

Get a pointer to the options used to create an ESLIF instance, put in the value pointed by C<marpaESLIFOptionpp> if it is not C<NULL>. The returned pointer must I<never> be freed.
