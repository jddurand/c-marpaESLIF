...
This documentation is a tutorial with a calculator as example.
...
The grammar is set in a constant:
...
and main program will instantiate a generic logger:
...
which means that the whole grammar start at C<bytep>,
that it is of length C<bytel>, and
that you should (please) manage the grammar encoding yourself.
...
Obviously, we would like the grammar to discard non-significant I<whitespace> automatically, as is traditional in most languages.
The statement
...
Here C<[1]> is the index, relative to the first RHS.
...
At this stage, if we do not instantiate any valuation, it is perfectly okay to have no action implementation.
Indeed, we can try immediately to parse some input.
For example one that is compliant with the grammar.
Since marpaESLIF is working natively in stream mode, we have to have a I<reader>.
In our case will send the whole chunk of input in one go:
