...
And ask for a scan of what it is C<stdin> (we assume that 4096 bytes is enough -;):
...
Output looks like (we removed a timestamp generated by the generic logger -;):
  ...
  ERROR [F1@0..3] Expression ::= Expression[0] .
  ERROR [F3@2..3] Expression[1] ::= Expression[2] .
  ERROR [F4@2..3] Expression[2] ::= Expression[3] .
  ERROR [F5@2..3] Expression[3] ::= /[\d]+/ .
  ERROR [F10@0..3] Expression[0] ::= Expression[0]
  ERROR [F10@0..3]                   '+'
  ERROR [F10@0..3]                   Expression[1] .
  ERROR [R7@2..3] Expression[2] ::= Expression[3]
  ERROR [R7@2..3]                   . '**'
  ERROR [R7@2..3]                   Expression[2]
  ERROR [R8@2..3] Expression[1] ::= Expression[1]
  ERROR [R8@2..3]                   . '*'
  ERROR [R8@2..3]                   Expression[2]
  ERROR [R9@2..3] Expression[1] ::= Expression[1]
  ERROR [R9@2..3]                   . '/'
  ERROR [R9@2..3]                   Expression[2]
  ERROR [R10@0..3] Expression[0] ::= Expression[0]
  ERROR [R10@0..3]                   . '+'
  ERROR [R10@0..3]                   Expression[1]
  ERROR [R11@0..3] Expression[0] ::= Expression[0]
  ERROR [R11@0..3]                   . '-'
  ERROR [R11@0..3]                   Expression[1]
  ERROR --------------------------------------------
  ...
  ERROR Expected terminal: '-'
  ERROR --------------------------------------------
  ERROR UTF-8 converted data before the failure (6 bytes)
  ERROR 0x000000: 31 20 2b 20 32 20                               1 + 2           
  ERROR --------------------------------------------
  ERROR <<<<<< RECOGNIZER FAILURE AFTER LINE No 1 COLUMN No 6, HERE: >>>>>>
  ERROR --------------------------------------------
...
will output nothing. And this is normal. From the grammar point of view, even if parsing is not complete, parsing did not fail: the input was conform to the grammar up to its end. This is by doing valuation that we will discover there is a problem.
