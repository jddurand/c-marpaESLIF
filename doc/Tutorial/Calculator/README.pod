...
where C<userDatavp> is the end-user valuation context, C<contexti> is a free context number, that must be > 0 when it comes outside from the ESLIF engine. Our actions always pushed only an integer or a double, so we only to provide the C<intTransformerp> and <doubleTransformerp> implementations:
