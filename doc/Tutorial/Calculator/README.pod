...
No output. Good, parsing was ok. Now on something that is not ok:
  ...
  echo xxxxxxx | /tmp/Calculator

Output look like (we removed a timestamp generated by the generic logger -;):

 ERROR --------------------------------------------
 ERROR Recognizer failure. Current state:
 ERROR [P1@0..0] Expression ::= . Expression[0]
 ERROR [P2@0..0] Expression[0] ::= . Expression[1]
 ERROR [P3@0..0] Expression[1] ::= . Expression[2]
 ERROR [P4@0..0] Expression[2] ::= . Expression[3]
 ERROR [P5@0..0] Expression[3] ::= . /[\d]+/
 ERROR [P6@0..0] Expression[3] ::= . '('
 ERROR [P6@0..0]                   Expression[0]
 ERROR [P6@0..0]                   ')'
 ERROR [P7@0..0] Expression[2] ::= . Expression[3]
 ERROR [P7@0..0]                   '**'
 ERROR [P7@0..0]                   Expression[2]
 ERROR [P8@0..0] Expression[1] ::= . Expression[1]
 ERROR [P8@0..0]                   '*'
 ERROR [P8@0..0]                   Expression[2]
 ERROR [P9@0..0] Expression[1] ::= . Expression[1]
 ERROR [P9@0..0]                   '/'
 ERROR [P9@0..0]                   Expression[2]
 ERROR [P10@0..0] Expression[0] ::= . Expression[0]
 ERROR [P10@0..0]                   '+'
 ERROR [P10@0..0]                   Expression[1]
 ERROR [P11@0..0] Expression[0] ::= . Expression[0]
 ERROR [P11@0..0]                   '-'
 ERROR [P11@0..0]                   Expression[1]
 ERROR --------------------------------------------
 ERROR Expected terminal: /[\d]+/
 ERROR Expected terminal: '('
 ERROR <<<<<< RECOGNIZER FAILURE HERE: >>>>>>
 ERROR --------------------------------------------
 ERROR Raw data after the failure (8 bytes)
 ERROR 0x000000: 78 78 78 78 78 78 78 0a                         xxxxxxx.        
 ERROR --------------------------------------------
