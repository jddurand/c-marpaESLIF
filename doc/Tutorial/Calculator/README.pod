...
You see that we have implemented two transformers: at the end of the valuation, marpaESLIF I<will> call for transformation before giving up. This is how we get the final value.
...
The final value requires a "transformer", that will be called internally after valuation is ok. This transformer is a set of function pointers, the end-user is responsible to provide the correct function pointer v.s. the type of the final value:
...
where C<userDatavp> is the end-user valuation context, C<contextp> is a free context pointer, that must be != C<NULL> when it comes outside from the ESLIF engine. Our actions always pushed only an integer or a double, so we only have to provide the C<intTransformerp> and <doubleTransformerp> implementations:
...
Here is the final version as can be found in the marpaESLIF repository, where we changed:

  '(' Expression ')'              assoc => group action => ::copy[1]

to

   # Parenthesis are scanned but skipped during valuation
