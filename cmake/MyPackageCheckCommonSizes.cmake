    ...
    FOREACH (_size 8 16 32 64)
      ...
      FOREACH (_c "char" "short" "int" "long" "long long")
        ...
        IF (("${CMAKE_C_COMPILER_ID} x" MATCHES "MSVC") OR MSVC)
          IF (_size EQUAL 8)
            SET (_extension "${_sign}i8")
          ELSEIF (_size EQUAL 16)
            SET (_extension "${_sign}i16")
          ELSEIF (_size EQUAL 32)
            SET (_extension "${_sign}i32")
          ELSEIF (_size EQUAL 64)
            SET (_extension "${_sign}i64")
          ELSE ()
        ...
        IF (HAVE_SIZEOF_${_C})
          IF (${SIZEOF_${_C}} EQUAL ${_sizeof})
            ...
            IF (_size EQUAL 8)
              IF (_sign STREQUAL "")
                SET (${_MYTYPEMIN} "(-127${_extension} - 1)")
                SET (${_MYTYPEMAX} "127${_extension}")
              ELSE ()
                SET (${_MYTYPEMIN} "0x00U${_extension}")
                SET (${_MYTYPEMAX} "0xFFU${_extension}")
              ENDIF ()
            ELSEIF (_size EQUAL 16)
              IF (_sign STREQUAL "")
                SET (${_MYTYPEMIN} "(-32767${_extension} - 1)")
                SET (${_MYTYPEMAX} "32767${_extension}")
              ELSE ()
                SET (${_MYTYPEMIN} "0x0000U${_extension}")
                SET (${_MYTYPEMAX} "0xFFFFU${_extension}")
              ENDIF ()
            ELSEIF (_size EQUAL 32)
              IF (_sign STREQUAL "")
                SET (${_MYTYPEMIN} "(-2147483647${_extension} - 1)")
                SET (${_MYTYPEMAX} "2147483647${_extension}")
              ELSE ()
                SET (${_MYTYPEMIN} "0x00000000U${_extension}")
                SET (${_MYTYPEMAX} "0xFFFFFFFFU${_extension}")
              ENDIF ()
            ELSEIF (_size EQUAL 64)
              IF (_sign STREQUAL "")
                SET (${_MYTYPEMIN} "(-9223372036854775807${_extension} - 1)")
                SET (${_MYTYPEMAX} "9223372036854775807${_extension}")
              ELSE ()
                SET (${_MYTYPEMIN} "0x0000000000000000U${_extension}")
                SET (${_MYTYPEMAX} "0xFFFFFFFFFFFFFFFFU${_extension}")
              ENDIF ()
            ELSE ()
              MESSAGE(FATAL_ERROR "Unsupported size ${_size}")
            ENDIF ()
