MACRO (MYPACKAGEDEPENDENCY packageDepend packageDependSourceDir)
  #
  # Optional argument:
  # PUBLIC   : Sets dependency scope to PUBLIC (default)
  # PRIVATE  : Sets dependency scope to PRIVATE
  # INTERFACE: Sets dependency scope to INTERFACE
  # STATIC   : Sets dependency to ${packageDepend}_static, else this is ${packageDepend}. Implies PRIVATE scope.
  # TESTS    : Adds packageDepend to test targets
  # LIBS     : Adds packageDepend to lib targets
  # EXES     : Adds packageDepend to exe targets
  #
  SET (_SCOPE "PUBLIC")
  SET (_TESTS FALSE)
  SET (_LIBS FALSE)
  ...
  FOREACH (_var ${ARGN})
    IF (${_var} STREQUAL PUBLIC)
      IF (MYPACKAGE_DEBUG)
        MESSAGE (STATUS "[${PROJECT_NAME}-DEPEND-DEBUG] ${packageDepend} public scope")
      ENDIF ()
      SET (_SCOPE "PUBLIC")
    ENDIF ()
    IF (${_var} STREQUAL PRIVATE)
      IF (MYPACKAGE_DEBUG)
        MESSAGE (STATUS "[${PROJECT_NAME}-DEPEND-DEBUG] ${packageDepend} private scope")
      ENDIF ()
      SET (_SCOPE "PRIVATE")
    ENDIF ()
    IF (${_var} STREQUAL INTERFACE)
      IF (MYPACKAGE_DEBUG)
        MESSAGE (STATUS "[${PROJECT_NAME}-DEPEND-DEBUG] ${packageDepend} interface scope")
      ENDIF ()
      SET (_SCOPE "INTERFACE")
    ENDIF ()
    IF (${_var} STREQUAL STATIC)
      IF (MYPACKAGE_DEBUG)
        MESSAGE (STATUS "[${PROJECT_NAME}-DEPEND-DEBUG] ${packageDepend} static linking")
      ENDIF ()
      SET (_STATIC TRUE)
      SET (_SCOPE "PRIVATE")
    ENDIF ()
  ...
  FOREACH (_target ${_candidates})
    ...
    IF (TARGET ${_target})
      ...
      IF (_STATIC)
        ...
        ENDIF ()
        TARGET_COMPILE_DEFINITIONS(${_target} ${_SCOPE} -D${packageDepend}_STATIC)
        IF (MYPACKAGE_DEBUG)
      ...
      IF (ALL_IN_ONE)
        ...
        IF (TARGET ${realPackageDepend})
          ...
          ENDIF ()
          TARGET_LINK_LIBRARIES(${_target} ${_SCOPE} ${realPackageDepend})
          IF (_TESTS)
            #
        ...
        ELSE ()
          ...
          FOREACH (_include_directory ${_packageDepend_fake_include_dirs})
            ...
            ENDIF ()
            TARGET_INCLUDE_DIRECTORIES(${_target} ${_SCOPE} ${_include_directory})
            #
      ...
      ELSE ()
        ...
        FOREACH (_library ${_property})
          ...
          ENDIF ()
          TARGET_LINK_LIBRARIES(${_target} ${_SCOPE} ${_library})
        ENDFOREACH ()
        #
        # Compile definitions
        #
        ...
        FOREACH (_flag ${_property})
          ...
          ENDIF ()
          TARGET_COMPILE_DEFINITIONS(${_target} ${_SCOPE} ${_library})
        ENDFOREACH ()
        #
        # Link flags
        #
