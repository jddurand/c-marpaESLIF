...
Clone a yet existing C<marpaWrapperGrammarOriginp> grammar. This clone is guaranteed to have the same symbol and rule ids, event and start symbol settings as the original at the time of this call. A cloned grammar is never precomputed.
