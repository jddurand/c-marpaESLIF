...
Get the Length Earleme Set Id in the value pointed by C<lengthip>, if it is not C<NULL>.
...
=head2 marpaWrapperAsf_prunedValueb and marpaWrapperAsf_prunedValueb
...
Return 0 in case of failure, a positive value if success, a negative value if reject. It is called for the rule number C<rule>. Context in terms of parent rule IDs is in <parentRuleiStackp>. C<rulei> is the rule number, C<arg0i> and C<argni> are the stack indices.

=item C<okSymbolCallackp>

Return 0 in case of failure, a positive value if success, a negative value if reject. It is called for the symbol number C<symboli> and the input at indice C<argi> in the input stack. Context in terms of parent rule IDs is in <parentRuleiStackp>. C<symboli> is the symbol number, and C<argi> is the stack number.

=item C<okNullingCallackp>

Return 0 in case of failure, a positive value if success, a negative value if reject. It is called for the nulling symbol number C<symboli>, per def there is no associated input. Context in terms of parent rule IDs is in <parentRuleiStackp>. C<symboli> is the symbol number.
...
Even if the ASF is instanciated with a false <ambiguousb> value, and the parse tree is ambiguous, it is then possible to get a single value from it. A typical usage of C<marpaWrapperAsf_prunedValueb> is a grammar that has a notion of I<exclusion>, e.g. EBNF.
...
=head2 marpaWrapperAsfValue_newp
  ...
  marpaWrapperAsfValue_t *marpaWrapperAsf_valueb(marpaWrapperAsf_t                    *marpaWrapperAsfp,
                                                 void                                 *userDatavp,
                                                 marpaWrapperAsfOkRuleCallback_t       okRuleCallbackp,
                                                 marpaWrapperAsfOkSymbolCallback_t     okSymbolCallbackp,
                                                 marpaWrapperAsfOkNullingCallback_t    okNullingCallbackp,
...
This method instanciates a valuation opaque pointer that will allow the user to use ASF as if it was a marpaWrapperValue instance. All parameters have exactly the same semantic as for marpaWrapperAsf_prunedValueb().
...
Another example of how to use the ASF API. This method simulate the marpaWrapperValue_valueb() behaviour, but using the ASF, i.e. it returns -1 on failure, 0 when there is no more parse value, and 1 on success.
