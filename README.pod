...
Exceptions are lua calls that will do an abort: if you look to the documentation, despite they have a return code for semantic purpose, they will I<never> return: they exist only for the purpose of generating an exception. These functions are:
...
The corresponding luaunpanic_wrappers exist solely for coherency but they will wrap nothing because the semantic of breaking the program must not change, i.e. the wrappers are proxy macros:
...
In case a luaunpanic method fails, which should be extremely rare, a new method exist when the wrapper itself panics:
