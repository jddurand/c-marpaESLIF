...
genericHash is a basic perl-like generic hash, perl-like means that for a single key there is a single value. It is writen entirely with macros, and is generic in the sense that hashing function and comparison function are left to the end-user.

=head1 SYNPOSIS

  genericStack_t myStack, *myStackp = &myStack;
