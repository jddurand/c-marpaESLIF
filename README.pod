...
Set C<var>, that is an internal structure containing both a pointer and a length.
...
=item GENERICSTACKITEMTYPE_ARRAY_PTR(a)

Pointer associated to the array. Can be an l-value.

=item GENERICSTACKITEMTYPE_ARRAY_LENGTH(a)
...
This genericStack does not manipulate any memory but the one it need.
