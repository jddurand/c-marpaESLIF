...
This generic stack implementation supports indices that are I<expressions>. The side-effect is that it is recommended to not use more than one generic stack macro in statements where the order of execution is undefined. Basically, this mean: use one generic stack macro at a time.
