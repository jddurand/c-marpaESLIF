...
=head2 marpaWrapperAsf_traverse_rh_valuesStackp

  genericStack_t *marpaWrapperAsf_traverse_rh_valuesStackp(marpaWrapperAsfTraverser_t *traverserp);

Returns a stack of all current RHS values, a C<NULL> value in case of faillure. In particular this will fail if current node is a token.

=head2 marpaWrapperAsf_traverse_rh_allChoicesStackp

  genericStack_t *marpaWrapperAsf_traverse_rh_allChoicesStackp(marpaWrapperAsfTraverser_t *traverserp)

Returns a new result list produced by taking a Cartesian product of the parse results (which are a list of choices) at the current position, C<NULL> in case of faillure.
