...
=head2 genericLogger_newp(logCallbackp, userDatavp, genericLoggerLeveli)
...
=head2 genericLogger_clonep(genericLoggerp)
...
=head2 genericLogger_freev(genericLoggerpp)

Free the generic logger.

=head2 genericLogger_logv(genericLoggerp, genericLoggerLeveli, fmts, ...)

Format the message using C<fmts> format string and eventual remaining parameters, and send it to the log implementation.

=head2 void genericLogger_logapv(genericLoggerp, genericLoggerLeveli, fmts, ap);

va_list version of genericLogger_logv().

=head2 genericLogger_logLevel_seti(genericLoggerp, logLeveli)

Set the log level to C<leveLeveli> and return it.

=head2 genericLogger_logLevel_geti(genericLoggerp)
