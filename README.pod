...
Generate an MARPAWRAPPERGRAMMAR_EVENT_EXHAUSTED event when parse tree is exhausted. Unless this option is set, an info message is always generated.
...
Returns the number of RHS symbols at the current node, -1 in case of failure. In particular, this will fail if it is called for a token. A Nulling rule returns a zero length.
...
If it is returns a true value, then the user-space value of RH symbol C<rhIxi> in filled in C<*valueip>.
...
Return 0 in case of failure, a positive value if success, a negative value if reject. It is called for the rule number C<rule>. Context in terms of parent rule IDs is in <parentRuleiStackp>.
...
Return 0 in case of failure, a positive value if success, a negative value if reject. It is called for the nulling symbol number C<symboli>, per def there is no associated input. Context in terms of parent rule IDs is in <parentRuleiStackp>.
