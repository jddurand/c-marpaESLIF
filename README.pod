...
An example of how to use the ASF API. This method simulate the value namespace, by pruning the ASF using the C<okRuleCallack> function pointer. C<okRuleCallack> is called with the stack of parent rule IDs in C<parentRuleiStackp>, and for the current rule in C<rulei>. Even if the ASF is instanciated with a false <ambiguousb> value, it is then possible to get a single value from an ambiguous parse. A typical usage of C<marpaWrapperAsf_valueb> is a grammar that has a notion of I<exclusion>, e.g. EBNF. This method will not accept that the <okRuleCallbackp> return a true more than once is there are more than once alternative at a certain point in the ASF.

Returns a true value on success, a false value on failure. In case of success, C<*valueip> will contain the indice in the output stack that have the final result.
