...
discard is yet another symbol in any grammar, but with special semantic. For example:

  :discard   ::= whitespace
  whitespace   ~ /[\s]+/

As soon as there is no match, if discard exist, any sub-recognizer will try to get a match on it. Nothing distinguishes the special symbol I<:discard> from the others: it can reference as well any symbol in any other sub-grammar. Though a discard match tentative cannot instanciate another discard match from a sub-grammar. This mean that in the following:
...
if a discard tentative is instance on grammar at level 0 using the C<whitespace> of level 1, and if C<whitespace> of level 1 does not match, there will be no tentative for discard in level 1 using its symbol I<somethingelse>.
