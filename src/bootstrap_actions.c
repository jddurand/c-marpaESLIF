...
/*****************************************************************************/
{
  if (singleSymbolp != NULL) {
    ...
    if (singleSymbolp->modifiers != NULL) {
      free(singleSymbolp->modifiers);
    }
    free(singleSymbolp);
  }
}
...
/*****************************************************************************/
{
  ...
  for (i = 0; i < GENERICSTACK_USED(grammarp->symbolStackp); i++) {
    ...
    /* Modifiers ? */
    if ((quotedStringp->modifiers != NULL) && (symbolp->u.terminalp->modifiers == NULL)) {
      continue;
    }
    if ((quotedStringp->modifiers == NULL) && (symbolp->u.terminalp->modifiers != NULL)) {
      continue;
