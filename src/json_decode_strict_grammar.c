static const char *marpaESLIFJSON_decode_strict_grammars =
  ...
  ":terminal ::= '}'                                                                                pause => after event => 1_dec$\n"
  "\n"
  ":terminal ::= /(?:[^\"\\\\\\x00-\\x1F]+)|(?:\\\\[\"\\\\\\/bfnrt])|(?:(?:\\\\u[[:xdigit:]]{4})+)/ pause => after event => 2_CHAR$ name => CHAR\n"
  ":terminal ::= /true|false|null|(?:-?(?:0|[1-9][0-9]*)(?:\\.[0-9]+)?(?:[eE][+-]?[0-9]+)?)/        pause => after event => 3_CONSTANT_OR_NUMBER$ name => CONSTANT_OR_NUMBER\n"
