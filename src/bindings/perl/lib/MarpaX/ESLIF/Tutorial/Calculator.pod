...
Tare care with rules default action though: it is C<::concat> so this will try to concatenate all the RHSs. If one the RHSs is a user-defined value, then MarpaX::ESLIF cannot know how to represent it, so this will be silently dropped when doing the concatenation. The exception is when there is a single RHSs: rule's C<::concat> is forced to be equivalent to C<::shift>, that is guaranteed to not change the nature of the value being transfered from the right-hand side of the rule to the left-hand side.
...
Scanner events are not generating by the grammar itself, but by the recognizer, and they always concern the lexemes: parsing can be paused before a lexeme is I<seen>, or after it has been I<seen> (though not yet commited). Pauses after a lexeme is seen can always be simulated with a nulled grammar event, in contrary to pauses before a lexeme is seen. The syntax is always:
  ...
  :lexeme ::= lexemeSymbol pause => before event => eventName

or

  :lexeme ::= lexemeSymbol pause => after event => eventName
...
At any time where the control is given back to the user, user-defined perl values, that can be I<anything>, can be injected. The most common method is:
  ...
  $eslifRecognizer->lexemeRead(symbolName, perlValue, lengthInBytes, grammarLength)
...
Since injecting values is possible only for lexemes, it is often practical to require a pause information from the recognizer with the method:
  ...
  $eslifRecognizer->lexemeLastPause(symbolName)
...
You may rememember that we said that the default value for a rule is always to concatenate the input. And indeed, if we remove all the actions from the grammar above, it will try to concatenate all the lexemes and alternatives together, that would leave to this output:

  (1**2+2**2)*3**2

i.e. the parse tree result is the concatenation of all the valid input, this is a neat way to remove automatically C<:discard> rules from the input -;

You have to notice also that I<nothing> obliges you to inject a alternative value that would be of the same length of what I<should> be in the input: an alternative is totally is totally under user's responsbility. For instance, injecting the string C<*ANYTHING*> for the C<POW> token:
...
Always remember that rule default action is C<::concat>, that will silently drop everything coming from user-space (i.e. lexeme injection), I<except> when there is a single RHS: then C<::concat> is changed to <::shift> that is guaranteed to not change the nature of the RHS.

Also, for symbols, C<::concat> is an exact synonym for C<::shift>.

This is why the default action for both rules and symbols is: C<::concat>.
