...
Though please note that reading character per character, which means very few bytes per very few bytes without encoding information, can drive to a false encoding guess from MarpaX::ESLIF. Our input being for the moment pure ASCII, we could have avoid the guess with the C<encoding> method of the recognizer:
