...
C<scan()> can stop if there is an error, or if the parsing ended, or if there are events. To check for events you can the recognizer's C<events()> method, that return a reference to an array of hashes:

  my $eventsRef = $eslifRecognizer->events();
  use Data::Dumper;
  print "Events are scan():\n" . Dumper($eventsRef);

So let's introduce I<grammar> events, that can be of three types: prediction, nulled or completion.

=head4 Events on rules

  The syntax is: C<event event_name = event_type symbol>
