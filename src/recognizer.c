...
static inline short _marpaWrapperRecognizer_es_to_values(marpaWrapperRecognizer_t *marpaWrapperRecognizerp, int earleySeti, int *indiceip, void **valuepp);
...
/****************************************************************************/
short marpaWrapperRecognizer_earleySetsToValueb(marpaWrapperRecognizer_t *marpaWrapperRecognizerp, int startEarleySeti, int lengthInParseLocationsi)
/****************************************************************************/
{
  const static char         funcs[]                 = "marpaWrapperRecognizer_es_to_input_span";
  genericLogger_t          *genericLoggerp;
  marpaWrapperGrammar_t    *marpaWrapperGrammarp;
  int                       starti;
  void                      *valuep;;
  Marpa_Earley_Set_ID       marpaLatestEarleySetIdi;
  int                       earleySetForFirstPositioni;
  int                       earleySetForLastPositioni;
  int                       earleySeti;
  int                       firstIndicei;
  int                       lastIndicei;
  ...
  if (startEarleySeti < 0) {
    MARPAWRAPPER_ERRORF(genericLoggerp, "Invalid startEarleySeti %d", startEarleySeti);
    goto err;
  }
  if (lengthInParseLocationsi <= 0) {
    MARPAWRAPPER_ERRORF(genericLoggerp, "Invalid lengthInParseLocationsi %d", lengthInParseLocationsi);
    goto err;
  }
  ...
  earleySetForFirstPositioni = startEarleySeti + 1;
  earleySetForLastPositioni  = startEarleySeti + lengthInParseLocationsi;

  if (earleySetForFirstPositioni > marpaLatestEarleySetIdi) {
    MARPAWRAPPER_ERRORF(genericLoggerp, "start (%d) is at or after latest_earley_set (%d)", startEarleySeti, marpaLatestEarleySetIdi);
    goto err;
  }
  if (earleySetForLastPositioni > marpaLatestEarleySetIdi) {
    MARPAWRAPPER_ERRORF(genericLoggerp, "end (%d + %d) is after latest_earley_set ($latest_earley_set)", startEarleySeti, lengthInParseLocationsi, marpaLatestEarleySetIdi);
    goto err;
  }

  /* We want to know where we are exactly to fake a valuation step */
  MARPAWRAPPER_TRACEF(genericLoggerp, funcs, "marpa_r_earleme(%p, %d)", marpaWrapperRecognizerp->marpaRecognizerp, startEarleySeti);
  earlemei = marpa_r_earleme(marpaWrapperRecognizerp->marpaRecognizerp, startEarleySeti);
  if (earlemei < 0) {
    MARPAWRAPPER_MARPA_G_ERROR(genericLoggerp, marpaWrapperRecognizerp->marpaWrapperGrammarp->marpaGrammarp);
    goto err;
  }
  
  if (_marpaWrapperRecognizer_es_to_values(marpaWrapperRecognizerp, earleySetForFirstPositioni, &firstIndicei, NULL) == 0) {
    goto err;
  }
  if (_marpaWrapperRecognizer_es_to_values(marpaWrapperRecognizerp, earleySetForLastPositioni, &lastIndicei, NULL) == 0) {
    goto err;
  }

  starti  = firstIndicei;
  lengthi = (lastIndicei + lastLengthi) - firstStartPositioni;
  
  /* Negative lengths are quite possible if the application has jumped around in the input */
  if (lengthi < 0) {
    MARPAWRAPPER_TRACE(genericLoggerp, funcs, "Negative length detected");
    lastLengthi = 0;
  }
  ...
  MARPAWRAPPER_TRACEF(genericLoggerp, funcs, "return 1, starti=%d lengthi=%d", starti, lengthi);
  return 1;
...
/****************************************************************************/
{
  const static char         funcs[]        = "_marpaWrapperRecognizer_es_to_span";
  genericLogger_t          *genericLoggerp = marpaWrapperRecognizerp->marpaWrapperRecognizerOption.genericLoggerp;
  int                       indicei;
  void                     *valuep;
  ...
  if (earleySeti <= 0) {
    ...
    if (marpa_r_earley_set_values(marpaWrapperRecognizerp->marpaRecognizerp, earleySeti, &indice, &valuep) < 0) {
      MARPAWRAPPER_MARPA_G_ERROR(genericLoggerp, marpaWrapperRecognizerp->marpaWrapperGrammarp->marpaGrammarp);
      goto err;
    }
  }
  ...
  MARPAWRAPPER_TRACEF(genericLoggerp, funcs, "return 1, indicei=%d valuep=%p", indicei, valuep);
  return 1;
