...
const static char *grammars = "# Parameterized grammar\n"
  ...
  "  local x2, y2 = table.unpack(marpaESLIFContextStackp:get())\n"
  "  print('[lua] x='..tostring(x)..', y='..tostring(y))\n"
  "  print('[lua] context.x='..tostring(context.x)..', context.y='..tostring(context.y))\n"
  "  print('[lua] context.X='..tostring(context.X)..', context.Y='..tostring(context.Y))\n"
  "  print('[lua] x2='..tostring(x2)..', y2='..tostring(y2))\n"
  "  error('JDD')\n"
  "end\n"
  "</luascript>\n"
  ;
