...
static const char *grammars =
  ...
  "# -----------------------------------------------------------------------------------------------------------\n"
  ":symbol ::= ','                                      name => COMMA                     priority =>  1\n"
  ":symbol ::= '{'                                      name => LBRACKET                  priority =>  1\n"
  ":symbol ::= '}'                                      name => RBRACKET                  priority =>  1\n"
  ":symbol ::= '['                                      name => LSQUARE                   priority =>  1\n"
  ":symbol ::= ']'                                      name => RSQUARE                   priority =>  1\n"
  ":symbol ::= /true|false|null/                        name => CONSTANT                  priority =>  1\n"
  ":symbol ::= /-?(?:0|[1-9][0-9]*)(?:\\.[0-9]+)?(?:[eE][+-]?[0-9]+)?/ name => NUMBER     priority =>  1\n"
  "#\n"
  "# A string containing only unescaped characters is considered more probable than one having escapes\n"
  "#\n"
  ":symbol ::= /\"[^\"\\\\\\x00-\\x1F]*\"/u             name => ONLY_UNESCAPED_CHARACTERS priority =>  1\n"
  ":symbol ::= '\"'                                     name => DQUOTE                    priority =>  0\n"
  "\n"
  "# Inside a string having escapes, it is anyway considered more probable to find more unescaped characters than the others\n"
  ":symbol ::= /[^\"\\\\\\x00-\\x1F]+/u                 name => UNESCAPED_CHARACTERS      priority => -1\n"
  "# Escaped characters are considered more probable than escaped unicode\n"
  ":symbol ::= /(?:\\\\[\"\\\\\\/bfnrt])+/u             name => ESCAPED_CHARACTERS        priority => -2\n"
  ":symbol ::= /(?:\\\\u[[:xdigit:]]{4})+/u             name => ESCAPED_UNICODE           priority => -3\n"
  "# ---------------------------------\n"
