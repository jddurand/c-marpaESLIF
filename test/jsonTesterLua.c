...
const static char *dsl = "\n"
  ...
  "# JSON Numbers\n"
  "# ------------\n"
  "number ::= NUMBER            # /* bignum */       action => ::lua->lua_number            # Prepare for eventual bignum extension\n"
  "\n"
  "NUMBER   ~ _INT\n"
  "         | _INT _FRAC\n"
  "         | _INT _EXP\n"
  ...
  "    local _hex = {}\n"
  "    local _length = string.len(u)\n"
  "    local _nextArrayIndice = 1\n"
  "\n"
  "    -- Per def u is a sequence of \\\\u[[:xdigit:]]{4} i.e. 6 'characters', ahem bytes\n"
  "    while (_length > 0) do\n"
  "      local _codepointAsString = string.sub(u, _length + 3, _length + 6) -- Extract the [[:xdigit:]]{4} part\n"
