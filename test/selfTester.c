...
const static char *selfs = "# Self grammar\n"
  ...
  "                                  | <rhs parameterized primary>\n"
  "<rhs parameterized primary>     ::= <single symbol> '->' '(' <lua explist maybe> ')'\n"
  "                                  | <symbol> '@' <grammar reference> '->(' <lua explist maybe> ')'\n"
  ...
  "<lua fieldlist>                 ::= <lua field>+ separator => <lua fieldsep> proper => 0\n"
  "<lua elseif exp then block>     ::= 'elseif' <lua exp> 'then' <lua block>\n"
  "<lua elseif exp then block any> ::= <lua elseif exp then block>*\n"
  ...
  "                                  | 'for' <lua Name> '=' <lua exp> ',' <lua exp> ',' <lua exp> 'do' <lua block> 'end'\n"
  "                                  | 'for' <lua Name> '=' <lua exp> ',' <lua exp>               'do' <lua block> 'end'\n"
  ...
  "<lua retstat>                   ::= 'return' <lua explist maybe>\n"
  "                                  | 'return' <lua explist maybe> ';'\n"
  ...
  "<lua funcname header>           ::= <lua Name>+ separator => '.' proper => 1\n"
  "<lua funcname>                  ::= <lua funcname header>\n"
  "                                  | <lua funcname header> ':' <lua Name>\n"
  "# <not parameterized lhs>         ::= <parameterized rhs> -> (1 + 2 + 3)\n"
  "\n"
  ":default ~ \n"
  "  regex-action => ::lua->LuaRegexAction\n"
