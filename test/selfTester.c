...
const static char *selfs = "# Self grammar\n"
  ...
  " */\n"
  "# :start ::= <start>\n"
  "# start ::= <lhs with rhs call>-->('x', 'y')\n"
  "start ::= <jdd 1>->('param1', 'param2') | <jdd 2>\n"
  "event ^jdd_1 = predicted <jdd 1><-(x,y)\n"
  "<jdd 1><-(x, y) ::= <jdd param>->(1)\n"
  "<jdd 2> ::= <jdd param>->(2)\n"
  "<jdd param><-(x) ::= 'JDD'\n"
  ":desc    ::= 'G1 TEST'\n"
  ":default ::=\n"
  ...
  "                                 | <terminal rule>\n"
  "                                 | <lhs with rhs call>-->(x + 1, 1)\n"
  "                                 | <lhs with rhs call>-->(x + 2, 2)\n"
  "<quantified test rule 1>       ::= 'JDD'* separator => <lhs with rhs call>-->(x + 3, 3)\n"
  "<quantified test rule 2>       ::= <lhs with rhs call>-->(x + 4, 4)* separator => 'JDD'\n"
  "<quantified test rule 3>       ::= <lhs with rhs call>-->(x + 5, 5)+ separator => <lhs with rhs call>-->(x + 6, 6)\n"
  ...
  "# ---------\n"
  "\n"
  "<anything up to newline><-(pString,pNumber) ::= <ANYTHING UP TO NEWLINE>\n"
  "                                              | 'JDD'\n"
  ...
  "<lhs with rhs call><-(pString, pNumber) ::= <anything up to newline>-->(pString .. '.', pNumber + 1)  \n"
  "<lhs with eof>                 ::= :eof\n"
  "\n"
  "<luascript>\n"
