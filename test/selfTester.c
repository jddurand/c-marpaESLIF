...
const static char *selfs = "# Self grammar\n"
  ...
  "                                                return true\n"
  "                                              end\n"
  "event ^lhs_with_rhs_call_2 = predicted <lhs with rhs call 2><-(x, y)\n"
  "event lhs_with_rhs_call_2$ = completed <lhs with rhs call 2><-(x, y)\n"
  ...
  "<lhs with rhs call>            ::= <anything up to newline>-->('a string', 0 )  \n"
  "<lhs with rhs call 2><-(pString, pNumber) ::= <anything up to newline>-->(pString .. '.', pNumber + 1)  \n"
  "<lhs with eof>                 ::= :eof\n"
  "<lhs with parameters><-(x, ...)::= :eof\n"
  "<lhs with parameters 2><-(x, ...)::= :eof* separator => :eol\n"
  "\n"
  "<luascript>\n"
