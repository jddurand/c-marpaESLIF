...
const static char *selfs = "# Self grammar\n"
  ...
  " * ****************\n"
  " */\n"
  ":symbol ::= [a-z] regex-action => ::lua->another_regex_action\n"
  ":symbol ::= /::luac?\\->function\\((?C1)/ pause => after event => :discard[switch] name => 'LUAC_FUNCTION' regex-action => ::lua->another_regex_action\n"
