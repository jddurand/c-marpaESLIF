...
const static char *selfs = "# Self grammar\n"
  ...
  "                                                return true\n"
  "                                              end\n"
  "event ^lhs_with_rhs_call_2 = predicted <lhs with rhs call><-(x, y)\n"
  "event lhs_with_rhs_call_2$ = completed <lhs with rhs call><-(x, y)\n"
  ":lexeme ::= <op declare top grammar> pause => after event => op_declare_top_grammar$\n"
  ":lexeme ::= <lhs with rhs call>->(x, y) pause => after event => lhs_with_rhs_call_2_lexeme$\n"
  ":lexeme ::= <statement>->(x) pause => after event => statement_lexeme$\n"
  "\n"
  "/*\n"
  " * ****************\n"
  " * Symbol settings:\n"
