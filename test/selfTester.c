...
const static char *selfs = "# Self grammar\n"
  ...
  " * ****************\n"
  " */\n"
  ":terminal ::= /::luac?\\->function\\(/ pause => after event => :discard[switch] name => 'LUAC'\n"
  ":terminal ::= /\\-\\-?>\\(/ pause => after event => :discard[switch]\n"
  ":terminal ::= /<\\-\\-?\\(/ pause => after event => :discard[switch]\n"
  ":lexeme ::= <lua funcbody after lparen>@=2 pause => after event => :discard[switch] verbose => 1 name => 'LUA FUNCBODY AFTER LPAREN'\n"
